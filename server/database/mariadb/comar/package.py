...
def postInstall(fromVersion, fromRelease, toVersion, toRelease):
    ...
    if not os.path.exists("/var/lib/mysql/mysql"):
        ...
        # Run MySQL
        subprocess.Popen(['/usr/bin/mysqld', '--user=mysql', '--skip-grant-tables', '--basedir=/usr', '--datadir=/var/lib/mysql', '--max_allowed_packet=8M', '--net_buffer_length=16K', '--socket=%s' % SOCKFILE, '--pid-file=%s' % PIDFILE])
